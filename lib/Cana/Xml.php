...
class Cana_Xml extends Cana_Model {
    ...
    public function __construct($xml = null, $params = null) {
        if (isset($params['dir'])) {
            $this->path($params['dir']);
        } else {
            $this->path('');
        }
        if (isset($xml)) {
            if (@file_exists($this->path().$xml.$this->_extension)) {
                $this->_contents = $this->open($xml);
            } else  {
                $this->_contents = $xml;
            }
    ...
    /**
     * The open file wrapper method
     *
     * @param    string        The filename to open
     */
    public function open($file) {
        return file_get_contents($this->path().$file.$this->_extension);
    }    
    
    /**
     * The parse xml text wrapper
     */
    public function parse() {
        $this->data($this->xml2array($this->_contents));
        $this->_contents = null;
        return $this;
    }
